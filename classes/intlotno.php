...
class IntLotno {
	...
	public static function getcountbyPOShipped($code,$po) 
	{
		...
		try
		{
			...
			{
				$dataset = $conn->query("SELECT sum(b.waferqty) as total FROM intlotno a inner join custlotno b  on a.custlot = b.custlotno and a.custcode = b.custcode WHERE a.custcode ='".$code."' and a.pono = '".$po."' and a.station = (SELECT top (1) [station] from processroute where forpacking = 1 and process = b.processcat)");
			}
			$counter = 0;
	...
	public static function GetDetails($intlot)
	{
		...
		try{
			...
			if ($conn->has_rows($dataset)) {
				...
				'waferrun' => strtoupper($row["waferrun"]),
				'brm' => $row["brm"],
