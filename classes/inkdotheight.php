...
class InkDotHeight {
    ...
    public function AddInkDotHeight(){
		$conn = new Connection();
        $success = true;
		try{
			//$conn->open();
			...
			$con = $conn->open();
            $sql = "INSERT INTO dbo.inkdotheight (intlotno,custcode,waferno,p1,p2,p3,p4,p5,lastupdate,lastupdatedby,active) VALUES(?,?,?,?,?,?,?,?,?,?,?)";
	...
	public static function GetAllLogs()
	{
		...
		try{
			$conn->open();
			$dataset =  $conn->query("SELECT * from dbo.inkdotheight order by lastupdate asc");
			$counter = 0;
			include_once("user.php");
	...
	public function UpdateInkDotHeight($trackingno){
		$conn = new Connection();
        $success = true;
		try{
			//$conn->open();
			//$result = $conn->query("INSERT INTO dbo.PO (pono,custcode,qty,processcat,subprocesscat,status,lastupdate,lastupdatedby,active) VALUES('".$this->getpono()."','".$this->getcustcode()."','".$this->getqty()."','".$this->getprocesscat()."','".$this->getsubprocesscat()."','".$this->getstatus()."',NOW(),'".$this->getlastupdatedby()."',1)");
			$con = $conn->open();
            $sql = "UPDATE dbo.inkdotheight set waferno = ?, p1 = ?, p2 = ?, p3 = ?, p4 = ?, p5 = ? WHERE trackingno = ?";
