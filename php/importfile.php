...
if($_SESSION['idno'])
{
    if(isset($_POST["btnUpload"])){
        ...
        for($i=1;$i < (count($data)); $i++)
        {
            ...
            {
                try {
                    $exist = $upload->checkExist($custcode,trim(preg_replace('/[\t|\s{,}]/', '', $csv[0])));
                    if($exist == 'false')
                    {
                        $upload->setcustcode(trim($custcode));
                        $upload->setcustlotno(trim(preg_replace('/[\t|\s{,}]/', '', $csv[0])));
...
{
    header("location:../index.php?Error=Login session timeout!");
}
